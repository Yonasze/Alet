...
begin
  ...
  loop
    floor_token := case when floor_row.floor_number = 0 then 'G' else floor_row.floor_number::text end;
    ...
    loop
      generated_number := replace(
        replace(template_unit.unit_number_pattern, '{floor}', floor_token),
...
declare
  ...
  is_special boolean;
  typical_floor_start integer := coalesce((payload->>'typical_floor_start')::integer, 1);
  typical_floor_end integer := coalesce((payload->>'typical_floor_end')::integer, 1);
...
begin
  ...
  if typical_floor_start < 1
     or typical_floor_end < typical_floor_start
     or typical_floor_end > total_floor_count then
    raise exception 'Typical floor range must be between floor 1 and floor %', total_floor_count;
  end if;
  ...
  if exists (
    ...
    where ft.project_id = new_project.id and ft.name = 'Special Ground Floor'
    limit 1;
    ...
    insert into public.project_floors (
      ...
      target_org, new_project.id, 0, 'Ground Floor', 'special',
      special_template.id, false, null, 0
    );
  end if;
