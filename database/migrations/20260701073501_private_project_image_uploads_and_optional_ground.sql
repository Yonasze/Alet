...
drop policy if exists "project media upload own folder" on storage.objects;
create policy "project media upload own folder"
...
drop policy if exists "project media delete own folder" on storage.objects;
create policy "project media delete own folder"
...
select
  ...
  item->>'storage_path' as storage_path,
...
begin
  ...
  if jsonb_array_length(media_files) < 3 or jsonb_array_length(media_files) > 4 then
    raise exception 'Upload exactly 3 or 4 project images';
  end if;
  ...
  loop
    ...
    insert into public.project_media (
      ...
      storage_path, public_url, is_hero, is_public, is_approved,
      ...
      media_row.position = 1, true, should_publish,
      media_row.position - 1, target_user
    );
  end loop;
