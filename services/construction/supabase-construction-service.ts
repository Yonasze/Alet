...
export type ConstructionProject={id:string;name:string;code:string;phase:string;floors_completed:number;total_floors:number;expected_completion_date:string|null}
...
export async function getConstructionWorkspace(){
 ...
 request<ConstructionProject[]>('projects?select=id,name,code,phase,floors_completed,total_floors,expected_completion_date&order=name.asc'),
