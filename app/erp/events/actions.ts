...
export async function updateEventAction(_state: EventActionState, form: FormData): Promise<EventActionState> {
  try {
    const category = value(form,'category')
    const priority = value(form,'priority')
    if (!eventCategories.includes(category as never)) throw new Error('Choose a valid event category.')
    if (!eventPriorities.includes(priority as never)) throw new Error('Choose a valid priority.')
    await rpc('update_scheduled_event', {
      event_id: value(form,'event_id'), project_id: value(form,'project_id'), title: value(form,'title'),
      description: value(form,'description'), category, priority,
      starts_at: ethiopiaTimestamp(value(form,'starts_at')), ends_at: ethiopiaTimestamp(value(form,'ends_at')),
      all_day: form.get('all_day') === 'on', location: value(form,'location'), owner_id: value(form,'owner_id'),
      reminder_minutes: value(form,'reminder_minutes') || '60', attendee_ids: form.getAll('attendee_ids').map(String),
    })
    revalidatePath('/erp/events')
    return { success: 'Event updated and rescheduled.' }
  } catch (error) { return { error: error instanceof Error ? error.message : 'Unable to update event.' } }
}
